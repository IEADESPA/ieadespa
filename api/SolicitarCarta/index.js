...
module.exports = async function (context, req) {
  ...
  const ativa = await pool.request().input("id", sql.Int, matricula).input("tipo", sql.NVarChar(30), tipo)
    .query(`SELECT TOP 1 CartaId, Status FROM CartasTransito
            WHERE MembroId = @id AND Tipo = @tipo AND Status IN ('SOLICITADA','CONFIRMADA','EMITIDA')
  ...
  if (confirmar) {
    ...
    const declaracao = tipo === "MUDANCA"
      ? `Eu, ${nome}, solicito minha Carta de Mudança e estou ciente do meu desligamento do rol de membros da IEADESPA.`
      : `Eu, ${nome}, confirmo a solicitação desta Carta de Recomendação.`;
