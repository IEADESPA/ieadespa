...
module.exports = async function (context, req) {
  const chave = `versiculo-imagem:${ipDoPedido(req)}`;
  if (!permitir(chave)) {
    context.res = { status: 429, body: "Muitas tentativas. Aguarde alguns minutos." };
    return;
  }
  ...
  try {
    ...
    context.res = { status: 502, body: "Não foi possível gerar a imagem agora." };
    return;
  }
  if (!verse) {
    context.res = { status: 404, body: "Nenhum versículo encontrado." };
    return;
  }
